...
**Le bug était dans `admin-proxy` lui-même**, pas dans l'API IKOMA.

```javascript
// ❌ AVANT (bug) - ligne 241-242 de admin-proxy
const finalData = (typeof responseData === 'object' && responseData !== null)
  ? { ...responseData, ...baseResponse }  // SPREAD D'UN ARRAY → { 0: item0, 1: item1, ... }
  : responseData;
```

En JavaScript, `{ ...array }` transforme un array `[a, b, c]` en objet indexé `{ 0: a, 1: b, 2: c }`.

**Solution appliquée (v2) :**
```javascript
// ✅ APRÈS (fix) - wrapper générique corrigé
if (Array.isArray(responseData)) {
  // Arrays préservés : { items: [...], proxy_* }
...
Ce fix s'applique à **tous les endpoints** automatiquement :
...
└─────────────────┘                 └─────────────────┘                      └──────────────┘
        ↑                                    ↑
    Retourne des                      Préserve la structure
    arrays standards                  + ajoute proxy_* metadata
```
